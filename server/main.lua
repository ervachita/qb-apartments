...
-- İf you want more qb scripts =  https://discord.gg/wBNwPCv
...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    ExecuteSql(false, "INSERT INTO `apartments` (`name`, `type`, `label`, `citizenid`) VALUES ('"..apartmentId.."', '"..type.."', '"..label.."', '"..Player.PlayerData.citizenid.."')")
    TriggerClientEvent('mythic_notify:client:SendAlert', src, "Senin dairen var ("..label..")")
...
function CreateApartmentId(type)
	...
	while not UniqueFound do
		...
		ExecuteSql(true, "SELECT COUNT(*) as count FROM `apartments` WHERE `name` = '"..tostring(type .. AparmentId).."'", function(result)
...
function GetApartmentInfo(apartmentId)
    local retval = nil
    ExecuteSql(true, "SELECT * FROM `apartments` WHERE `name` = '"..apartmentId.."' ", function(result)
...
function GetOwnedApartment(citizenid)
    ExecuteSql(true, "SELECT * FROM `apartments` WHERE `citizenid` = '"..citizenid.."' ", function(result)
        if result[1] ~= nil then 
            return result[1]
        end
        return nil
...
ESX.TriggerServerCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
ESX.TriggerServerCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
ESX.TriggerServerCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
ESX.TriggerServerCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
        ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..cid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
        ...
        local src = source
        local Player = ESX.GetPlayerData(src)
        ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
...
ESX.TriggerServerCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = ESX.GetPlayerData(src)
    if Player ~= nil then
        ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
...
ESX.TriggerServerCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = ESX.GetPlayerData(src)

	if Player then
		ExecuteSql(false, "SELECT * FROM `player_outfits` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
			if result[1] ~= nil then
				cb(result)
			else
				cb(nil)
